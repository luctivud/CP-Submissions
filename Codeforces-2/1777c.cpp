...
void solveEachTest(int _TestCase) {
		...
		lld mxFac = *(se.rbegin());
		debugluc(se);
		forn1(i, (mxElem / mxFac)+1) { // m
			lld nowElem = mxFac * i;
			if (nowElem > mxElem) break;
			bool ok = true;
			lld mnGot = INF, mxGot = 0ll;
			for (lld el : se) {
				forn1(zz, (nowElem / el)+1) {
					lld want = (nowElem / el) * el;
					if (arr.find(want) != arr.end()) {
						mxGot = max(mxGot, want);
						mnGot = min(mnGot, want);
					} else {
						ok = false;
						break;
					}
