...
void solveEachTest(int _TestCase) {
    ...
    vector<lld> points(102, 0);

    forn(i, n) {
        points[arr[i]] += (i & 1 ? -1 : 1);
    }
    ...
    // _____error_____(points);
        
    bool ok = true;

    forn(i, m) {
        lld l, r; cin >> l >> r;
        _____error_____(points[r] - (l-1 >= 0 ? points[l-1] : 0ll));
        ok &= llabs(points[r] - (l-1 >= 0 ? points[l-1] : 0ll)) <= 1ll;
    }
