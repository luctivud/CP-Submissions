...
const lld d8i[8]={-1, -1, 0, 1, 1, 1, 0, -1}, 
...
bool possible (string s, lld i, lld j, lld ind, vector<vector<bool>> &visited) {
	if (ind == lld(s.length())) return true;
	if ((arr[i][j] != s[ind]) or visited[i][j]) {
		return false;
	}
	visited[i][j] = true;
	bool ok = false;
	forn(xx, 8) {
		if (isvalid(i+d8i[xx], j+d8j[xx])) {
			ok |= possible(s, i+d8i[xx], j+d8j[xx], ind+1, visited);
		}
	}
	return ok;
}
  ...
  forn(xx, sz) {
  	...
  	forn(i, n) {
  		forn(j, m) {
  			...
  			ok |= possible(words[xx], i, j, 0, visited);
  		}
  	}
  	if (ok) {
  		ans.insert(words[xx]);
