...
void solveEachTest(int _TestCase) {
    ...
    lld delta = 0ll, lastDone = 0ll;
    _____error_____(arr);
    priority_queue<array<lld, 3>> pQue;
    forn(i, n) {
        lld utregaAt = arr[i][1] + delta;
        lld itnaWant = arr[i][0];
        if (lastDone + itnaWant <= utregaAt) {
            pQue.push({itnaWant, lastDone, arr[i][2]});
            lastDone += itnaWant;
        } else if (pQue.size() and pQue.top()[0] > itnaWant) {
            delta += pQue.top()[0];
            pQue.pop();
            pQue.push({itnaWant, lastDone, arr[i][2]});
            lastDone += itnaWant;
    ...
    // NonIterable.print(pQue);

    vector<array<lld, 3>> ans;
    while (pQue.size()) {
    ...
    cout << ans.size() << "\n";
    for (auto [sz, st, ind] : ans) {
        cout << ind << " " << lastDone << " " << lastDone + sz << "\n";
        lastDone += sz;
    }
