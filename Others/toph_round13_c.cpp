...
void solveEachTest(int _TestCase) {
  ...
  sort(all(arr));
  string A = "Add Light Bulbs", 
  			 B = "Remove Light Bulbs";

  lld sumOfBulbs = accumulate(all(arr), 0ll);
  arr.pb(sumOfBulbs / n);
  arr.pb((sumOfBulbs + n - 1) / n);

  if (n & 1) {
  	lld mid1 = n / 2;
  	lld addBulb = 0ll, removeBulb = 0ll;
  	forn(i, n) {
  		addBulb += max(0ll, arr[mid1] - arr[i]);
  		removeBulb += max(0ll, arr[i] - arr[mid1]);
  	}

  	lld addBulb1 = 0ll, removeBulb1 = 0ll;
  	mid1 = n;
  	forn(i, n) {
  		addBulb1 += max(0ll, arr[mid1] - arr[i]);
  		removeBulb1 += max(0ll, arr[i] - arr[mid1]);
  	}
  	addBulb = min(addBulb1, addBulb);
  	removeBulb = min(removeBulb1, removeBulb);

  	addBulb1 = 0ll, removeBulb1 = 0ll;
  	mid1 = n+1;
  	forn(i, n) {
  		addBulb1 += max(0ll, arr[mid1] - arr[i]);
  		removeBulb1 += max(0ll, arr[i] - arr[mid1]);
  	...
  	lld mid2 = n / 2 - 1;
  	lld addBulb1 = 0ll, removeBulb1 = 0ll;
  	lld addBulb2 = 0ll, removeBulb2 = 0ll;
  	...
  	forn(i, n) {
  		addBulb2 += max(0ll, arr[mid2] - arr[i]);
  		removeBulb2 += max(0ll, arr[i] - arr[mid2]);
  	}

  	lld addBulb = min(addBulb1, addBulb2);
  	lld removeBulb = min(removeBulb1, removeBulb2);
