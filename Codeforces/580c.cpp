...
template<class T> void print(T arr[])   { EACH(i, arr) {cout << i << " ";} }
...
lld solve(lld node, lld cats, bool visited[], vector<lld> adj[], lld arr[]) {
	// error(node);
	visited[node] = true;
	if (arr[node]) {
		cats++;
	...
	for (auto i : adj[node]) {
		if (!visited[i]) {
			ok = false;
			res += solve(i, cats, visited, adj, arr);
		}
	}
...
void solveEachTest(lld _TestCase) {
    ...
    vector<lld> adj[n+1];
    forn(i, n-1) {
    	add_undirected_edge(read(_TestCase), read(_TestCase), adj);
    }

    bool visited[n+1] = {false};
    
    println(solve(1, 0, visited, adj, arr));
