...
void solveEachTest(int _TestCase) {
    ...
    priority_queue<pair<lld, char>> pq;
    for (auto [ke, va] : cnt) {
        pq.push({va, ke});
    }
    ...
    lld iind = 0, jind = 0;
    forn(i, n) {
        auto[cntChar, ch] = pq.top(); pq.pop();
        if (ch == s[i]) {
            if (pq.size() == 0) {
                break;
            }
            auto [cntChar2, ch2] = pq.top(); pq.pop();
            pq.push({cntChar, ch});
            ans[i] = ch2;
            if (cntChar2 > 1) {
                pq.push({cntChar2-1, ch2});
            }
        } else {
            ans[i] = ch;
            if (cntChar > 1) {
                pq.push({cntChar-1, ch});
            }
        }
        // NonIterable.print(pq);
        jind = i+1;
    ...
    while (jind < n) {
        while (s[iind] == s[jind] or s[jind] == ans[iind]) {
            iind++;
        }
        swap(ans[iind], ans[jind]);
        jind++;
    }
