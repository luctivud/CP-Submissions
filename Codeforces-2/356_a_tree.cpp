...
#define  forn(I7, E4) for(lld I7=0ll; I7 < E4; (I7)+=1)
#define forn1(I7, E4) for(lld I7=1ll; I7 < E4+1; (I7)+=1)
...
set<lld> merge_func(set<lld> a, set<lld> b) {
    set<lld> res{a};
    for (auto el : b) {
        res.insert(el);
    }
...
void solveEachTest(int _TestCase) {
    ...
    vector<set<lld>> arr(n);
    forn(i, n) {
        arr[i].insert(i);
    }

    segment_tree segg(arr, merge_func, set<lld>());
    ...
    forn(qq, q) {
        ...
        cin >> l >> r >> x;
        --l, --r, --x;
        set<lld> res = segg.query(l, r+1);
        _____error_____(res);
        for (lld el :  res) {
            if (el != x) {
                ans[el] = x;
                segg.set((int)el, set<lld>());
            }
        }
    }
