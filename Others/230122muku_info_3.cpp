...
int CalcBestAns(int n, int k, int a, int b, vector<int> A) {
	long long ans = 0;
	int mod = 1e9 + 7;
	...
	vector<long long> suffDiffInd(n+1, 0);
	for (int i = n-1; i >= 0; i--) {
		(suffSum[i] += (suffSum[i+1] + arr[i].first));
		(suffDiffInd[i] += (suffDiffInd[i+1] + abs(arr[i].second - k)));
	}

	int pos = 0;
	while (pos < n and arr[pos].second != k) {
		pos += 1;
	}
	if (pos == n) {
		return ans;
	...
	ans = 1e18;
	long long currHeight = arr[pos].first;
	while (pos < n) {
		long long cost = 0;
		cost += a * (arr[pos].first - currHeight);
		cost += b * (suffSum[pos+1] - (arr[pos].first * (n-pos-1)));
		cost += 2 * suffDiffInd[pos+1];
		ans = min(ans, cost);
		pos += 1;
	}
	return ans % mod;
