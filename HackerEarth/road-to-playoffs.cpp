...
void solveEachTest(int _TestCase) {
	// cout << "Case #" << _TestCase << ": ";
	lld n, m, k, b; read(n, m, k, b);
	vector<lld>arr(n); 
	forn(i, n) read(arr[i]);
	sort(all(arr));

	lld l = 0, r = n-1, ans = -1ll;
	while(l <= r) {
		lld mid = l + (r - l) / 2;
		vector<lld> brr{arr};
		brr[mid] += m;
		lld remainingPoints = (m * (k-1));
		for (lld ind = 0; (ind < mid) && remainingPoints > 0; ind++) {
			lld mx = min({remainingPoints, brr[mid] - brr[ind], m});
			remainingPoints -= mx; 
			brr[ind] += mx;
		}
		for (lld ind = mid+1; (ind < n) && remainingPoints > 0; ind++) {
			lld mx = min({remainingPoints, brr[mid] - brr[ind], m});
			remainingPoints -= mx; 
			brr[ind] += mx;
		}
		for (lld ind = n-1; ind >= 0 && remainingPoints > 0; ind--) {
			lld mx = min(remainingPoints, m-(brr[ind] - arr[ind]));
			brr[ind] += mx;
			remainingPoints -= mx;
		}
