...
lld solve(vector<lld> arr, lld i, vector<lld> &dp, lld mod, lld curr = 1ll, bool flag = false) {
	// error(i, curr);
	if (i == -1) {
	    if (!flag) return 0ll;
	    return curr % mod;
	...
	// if (arr[i] % mod == 0) return dp[i] = solve(arr, i-1, dp, mod, curr);
	// else if (arr[i] == 1) return dp[i] = solve(arr, i-1, dp, mod, curr, true);
	return dp[i] = max(solve(arr, i - 1, dp, mod, (curr * arr[i]), true),
	                   solve(arr, i - 1, dp, mod, curr, false));
}
...
void solveEachTest(lld _TestCase = 1) {
	// cout << "Case#" << _TestCase << ": ";
	lld n, m; read(n, m);
	vector<lld> inwhichgroup(n);
	read(inwhichgroup);
	vector<lld> groups[m];
	forn(i, n) {
		lld temp; read(temp);
		groups[inwhichgroup[i] - 1].pb(temp);
	}

	vector<lld> modulos(m);
	read(modulos);
	...
	forn(i, m) {
		lld sz = len(groups[i]);
		vector<lld> dp(sz, -1);
		print(solve(groups[i], sz - 1, dp, modulos[i]), "");
	}
