...
void solveEachTest(int _TestCase) {
    ...
    lld l = 0, r = 3*m, ans = 3*m;
    _____error_____(cntInd);
    while (l <= r) {
        lld mid = (l + r) / 2;
        lld remTime = 0ll, remWork = 0ll;
        forn1(i, n) {
            lld va = cntInd[i];
            remWork += max((va - mid), 0ll);
            remTime += max((mid - va) / 2, 0ll);
        }
        _____error_____(mid, remWork, remTime);
        if (remWork <= remTime) {
            ans = mid;
            r = mid - 1;
        } else {
