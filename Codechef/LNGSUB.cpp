...
void solveEachTest(int _TestCase) {
    ...
    vector<array<lld, 3>> weights;

    forn(i, n) {
        for (lld j = i+1; j < n; ++j) {
            weights.push_back({LCSubStr(arr[i], arr[j]), i, j});
        }
    }


    sort(rall(weights));
    // _____error_____(weights);
    ...
    for (auto [w, u, v] : weights) {
        if (find_repr(u, par3t, ran4k) != find_repr(v, par3t, ran4k)) {
            Union(u, v, par3t, ran4k);
            ans += w;
        }
    }
