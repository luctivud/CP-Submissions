...
void solveEachTest(int _TestCase) {
    ...
    // _____error_____(current);
    // _____error_____(scores);

    while(l <= r) {
        lld mid = l + (r - l) / 2;
        bool ok = true;
        for (lld i = mid; i < n; i++) {
            if (hisScore < current[i] + scores[n-i+mid-1]) {
                ok = false;
                break;
            }
        }
...
// lld hisScore = 0ll, ans = 1;

// while (i < n) {
//     if (i != ind) {
//         Riza[arr[i] + scores[j]] += 1;
//         i += 1;
//         j -= 1;
//     } else {
//         hisScore = arr[i] + scores[0];
//         Riza[arr[i] + scores[0]] += 1;
//         i += 1;
//     }
// }

// NonIterable.print(Riza);
