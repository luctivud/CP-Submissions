...
for i in range(n):
	...
	x **= 2
	x %= m
# print(ind, doneInd, prefix_sum[ind])
# print(prefix_sum)
# print(prefix_sum)

try:
	lastset = prefix_sum[-1] - prefix_sum[ind];
	offset = prefix_sum[ind]

	sz = len(prefix_sum) - ind - 1

	rem = n - ind
	ans = offset
	ans += lastset * (rem // sz)
	ans += prefix_sum[ind + (rem % sz)] - offset
# print(sz, lastset, rem)
except:
	ans = 1
