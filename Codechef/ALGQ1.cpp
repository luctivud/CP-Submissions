...
void solveEachTest(int _TestCase) {
		...
		vlld f(n); Read(f);
		lld m; Read(m);
		vector<array<lld, 3>> steps(m);
		forn(i, m) {
			Read(steps[i][0], steps[i][1], steps[1][2]);
		}

		DSU<100001> want;
		DSU<100001> has;

		sort(all(steps), [](auto &a, auto &b){
			return a[0] < b[0];
		});
		...
		vlld compSzWant(n+1, 0ll);
		vlld compSzHas(n+1, 1ll);
		lld nCompWant = 0ll;
		lld nCompHas = n;

		forn1(i, n) {
			compSzWant[want.find_repr(i)] += 1;
			nCompWant += (compSzWant[want.find_repr(i)] == 1);
		}

		lld ans = -1;
		if (nCompWant == nCompHas) {
			Print("0");
			return;
		}
		for (auto &[t, a, b] : steps) {
			lld repWantA = want.find_repr(a);
			lld repWantB = want.find_repr(b);
			nCompWant -= (repWantA != repWantB);
			lld repHasA = has.find_repr(a);
			lld repHasB = has.find_repr(b);
			has.Union(a, b);
			// check after operation
			if (nCompHas == nCompWant) {
				ans = t;
				break;
			}
		}
