...
void solveEachTest(int _TestCase) {
    // cout << "Case #" << _TestCase << ": ";
    lld n, m; cin >> n >> m;
    vector<pair<lld, lld>> queries(m);
    forn(i, m) {
        cin >> queries[i].f1 >> queries[i].s2;
    }
    ...
    lld doneFor = 1ll, remainingNum = n, ans = 0ll;

    forn(i, m) {
        doneFor = lcm(doneFor, queries[i].s2);
        lld okidoki = (n / doneFor);
        ans += (remainingNum - okidoki) * queries[i].f1;
        remainingNum = okidoki;
        if (doneFor > n) break;
        // _____error_____(doneFor, okidoki, remainingNum, ans);
    }
