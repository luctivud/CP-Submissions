...
void solveEachTest(int _TestCase) {
    ...
    lld firstApp = n+1, lastApp = -1;

    forn(i, n) {
        if (s[i] == '0') {
            firstApp = min(i, firstApp);
            lastApp = max(i,  lastApp);
        }
    }

    if (firstApp == n+1 and lastApp == -1) {
        cout << "1 " << 2*(n/2) << " " << "1 " << (n/2);
        return;
    }

    _____error_____(firstApp, lastApp);
    if (firstApp < (n - lastApp - 1)) {
        cout << firstApp + 1 << " " << n << " ";
        cout << firstApp + 2 << " " << n;
    } else {
        cout << 1 << " " << lastApp+1 << " ";
