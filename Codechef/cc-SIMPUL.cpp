...
void solveEachTest(lld _TestCase = 1) {
	// cout << "Case#" << _TestCase << ": ";
	lld n; read(n);
	map<lld, vector<lld>> values;
	map<lld, pair<lld, lld>> doneForRange;
	...
	lld q; read(q);
	for4(i, 0, q, 1) {
		lld l, r, x; read(l, r, x);
		l += offset; r += offset; x += offset;
		lld ans = 0ll;
		if (doneForRange.find((x)) == doneForRange.end()) {
			ans = lld(upper_bound(all(values[x]), r) - lower_bound(all(values[x]), l));
			doneForRange[x] = {l - 1, r + 1};
		} else {
			// print("ok");
			lld l1 = l, r1 = doneForRange[x].first, l2 = doneForRange[x].second, r2 = r;
			doneForRange[x] = {min(l1, r1) - 1, max(l2, r2) + 1};

			// error(l1, r1, l2 , r2);
			if (l1 < r1) {
				ans += lld(upper_bound(all(values[x]), r1) - lower_bound(all(values[x]), l1));
			}

			if (l2 < r2) {
				ans += lld(upper_bound(all(values[x]), r2) - lower_bound(all(values[x]), l2));
			}
