...
void solveEachTest(int _TestCase) {
    // cout << "Case #" << _TestCase << ": ";
    lld n, x, m; cin >> n >> x >> m;
    lld doneL = x, doneR = x, currPos = x;
    forn(i, m) {
        lld direction, distance; cin >> direction >> distance;
        if (direction) {
            currPos -= distance;
        } else {
            currPos += distance;
        ...
        doneL = min(doneL, max(1ll, currPos));
        doneR = max({doneR, min(n, currPos)});
        // assert(currPos > 0 and currPos <= n);
        _____error_____(currPos);
        _____error_____(doneL, doneR);
        cout << doneR - doneL + 1 << "\n";
    }
