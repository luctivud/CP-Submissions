...
void solveEachTest(int _TestCase) {
    ...
    _____error_____(indegree);

    lld ans = 0ll;
    vector<bool> visited(n + 1, false);
    vector<lld> orderOfVisited, negWeight;
    queue<lld> q;
    forn1(i, n) {
        if (indegree[i] == 0) {
            q.push(i);
    ...
    while (q.size()) {
        lld node = q.front(); q.pop();
        if (node == -1 or visited[node]) continue;
        visited[node] = true;
        ans += a[node];
        if (a[node] > 0) {
            orderOfVisited.push_back(node);
            if (b[node] != -1) {
                a[b[node]] += a[node];
            }
        } else {
            negWeight.push_back(node);
    ...
    cout << ans << "\n";
    for (auto i : orderOfVisited) {
        cout << i << " ";
    }
    for (auto i : negWeight) {
        cout << i << " ";
    }

    assert((lld)orderOfVisited.size() + (lld)negWeight.size() == n);
