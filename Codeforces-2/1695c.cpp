...
void solveEachTest(int _TestCase) {
    ...
    vector<vector<lld>> dpMax(n, vector<lld> (m, -inf)), dpMin(n, vector<lld> (m, inf));
    forn(i, n) {
        forn(j, m) {
            lld left = (j-1 >= 0 ? dpMax[i][j-1] : 0ll);
            lld up = (i-1 >= 0 ? dpMin[i-1][j] : 0ll);
            dpMax[i][j] = matr[i][j] + max(up, left);
            dpMin[i][j] = matr[i][j] + min(up, left);
        }
    }

    // NonIterable.print(dpMax);

    cout << ((dpMax[n-1][m-1]>=0 and dpMin[n-1][m-1]<=0) ? "YES" : "NO");
