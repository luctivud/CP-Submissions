...
perm = [0] + get_list()
cost = [0] + get_list()
# ans = max(0, sum(cost) * ((k) // n))
# print(ans)
# k = (k % n) + 1

connected_comp = defaultdict(list)
visited = [False] * (n+1)
comps = 1
for i in range(1, n+1):
	on_square = i
	if not visited[on_square]:
		# visited[on_square] = True
		# print(i)
		while not visited[on_square]:
			visited[on_square] = True
			connected_comp[comps].append(on_square)
			on_square = perm[on_square]
		comps += 1
# print(connected_comp)
ans = 0
for key, val in connected_comp.items():
	x1 = len(val)
	ans1 = max(0, (sum(cost[i] for i in val)) * (k // x1))
	x2 = k % x1
	ans2 = 0
	for i in val:
		on_square = i
		...
		for j in range(x2):
			on_square = perm[on_square]
			this_ans += cost[on_square]
			ans2 = max(ans2, this_ans)
