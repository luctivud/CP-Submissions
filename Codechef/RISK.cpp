...
void dfs(lld i, lld j, vector<vector<bool>> &visited, vector<string> arr, lld &sz) {
		if (arr[i][j] == '0' or visited[i][j]) return;
		// error(i, j);
		visited[i][j] = true;
		sz += 1;
		forn(xx, 4) {
			if (isvalid(i+d4i[xx], j+d4j[xx])) {
				dfs(i+d4i[xx], j+d4j[xx], visited, arr, sz);
			}
		}
		return;
}
