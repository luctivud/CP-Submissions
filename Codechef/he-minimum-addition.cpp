...
void solveEachTest(lld _TestCase) {
	...
	vector<vector<lld>> min_need_prefix(n + 1, vector<lld>(32, 0ll));

	forn1(i, n) {
		lld val = arr[i - 1];
		lld num = 1ll;
		forn(j, 32) {
			min_need_prefix[i][j] = min_need_prefix[i - 1][j];
			if (!(val & (1 << j))) {
				min_need_prefix[i][j] += (num) - (val % num);
			}
			num <<= 1;
			// val >>= 1;
		}
	...
	forn(qq, q) {
		...
		lld ans = INFll;
		forn(j, 32) {
			ans =  min(ans, min_need_prefix[b][j] - min_need_prefix[a][j]);
		}
...
vector<vector<lld>> prefix_bits(n + 1, vector<lld>(32, 0)) ;

	forn(i, n) {
		forn(j, 32) {
			prefix_bits[i + 1][j] = prefix_bits[i][j];
			if ((arr[i] >> j) & 1) {
				prefix_bits[i + 1][j] += 1;
			}
		}
	...
	forn(qq, q) {
		lld a, b; read(a, b);
		a--;
		vector<lld> currBits(32, 0ll);

		forn(i, 32) {
			currBits[i] = prefix_bits[b][i] - prefix_bits[a][i];
		}
		...
		forn(j, 32) {
			num += ((1 << j) * currBits[j]);
			print(num, "");
			ans = min(ans, max(0ll, ((sz) * (1ll << j)) - num));
		}
