...
void solveEachTest(lld _TestCase) {
	...
	set<lld> askedType1, askedType2;

	map<lld, lld> cntType1;

	lld ans = 0ll;

	vector<pair<lld, lld>> queries;
	...
	forn(qq, q) {
		...
		if (x == 1) {
			cntType1[num] += 1;
			// askedType1.insert(num);
		} else {
			if (!(cntType1[num] & 1)) {
				ans += cntType1[num];
			} else {
				ans += max(0ll, cntType1[num] - 2);
			}
			// askedType2.insert(num);
			cntType1[num] = 0;
		}
	}

	// EACH(it, askedType1) {
	// 	if (askedType2.find(it) == askedType2.end()) {
	// 		ans++;
	// 	}
	// }
	...
	for4(i, q - 1, -1, -1) {
		if (queries[i].f1 == 1) {
			if (askedType2.find(queries[i].s2) == askedType2.end()) {
				ans++;
			}
		} else {
			askedType2.insert(queries[i].s2);
		}
	}
