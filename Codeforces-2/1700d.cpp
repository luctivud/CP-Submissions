...
void solveEachTest(int _TestCase) {
    // cout << "Case #" << _TestCase << ": ";
    lld n; cin >> n;
    vector<lld> arr(n, 0ll), prefSum(n+1, 0ll), mxPref(n+1, 0ll);
    ...
    forn(i, n) {
        ...
        mxPref[i+1] = max(mxPref[i], arr[i]);
    }

    _____error_____(prefSum);
    _____error_____(mxPref);
    ...
    forn(qq, q) {
        ...
        while (l <= r) {
            lld mid = (l + r) / 2;
            bool ok = (mxPref[mid] <= t);

            lld netWaterGiven = t * mid;
