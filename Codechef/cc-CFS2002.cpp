...
void solveEachTest(lld _TestCase) {
	...
	vector<lld> prefix_sum {0}, cnt_odd, cnt_even;
	lld odd = 0ll, even = 0ll;
	map<lld, vector<lld> > pos;
	forn(i, n) {
		pos[arr[i]].pb(i);
		prefix_sum.pb(prefix_sum[i] + arr[i]);
		if (arr[i] & 1) {
			odd++;
		} else {
			even++;
	...
	EACH(it, pos) {
		...
		if (it.f1 & 1) {
			forn(i, sz - 1) {
				lld pos1 = ref[i];
				lld pos2 = ref[i + 1];
				lld numOdd = cnt_odd[pos2 - 1] - cnt_odd[pos1];
				if (numOdd & 1) {
					maxSum = max(maxSum, prefix_sum[pos2] - prefix_sum[pos1 + 1]);
				}
				// error(it.f1, pos1, pos2, numOdd, prefix_sum[pos2] - prefix_sum[pos1+1]);
			}
		} else {
			forn(i, sz - 1) {
				lld pos1 = ref[i];
				lld pos2 = ref[i + 1];
				lld numEven = cnt_even[pos2 - 1] - cnt_even[pos1];
				if (!(numEven & 1)) {
					maxSum = max(maxSum, prefix_sum[pos2] - prefix_sum[pos1 + 1]);
				}
				// error(it.f1, pos1, pos2, numEven, prefix_sum[pos2] - prefix_sum[pos1+1]);
			}
		}
	}
