...
void solveEachTest(int _TestCase) {
    ...
    forn(i, Q) {
    	read(query[i].f1, query[i].s2);
    	query[i].f1 -= 1;
    	auto it = se.find_by_order(query[i].f1);
    	int ans = 0ll;
		int num = query[i].s2;
		while (num and (it != se.end())) {
			int dist = *it - query[i].f1;
			int mn = min(num, arr[*it]);
			arr[*it] -= mn;
			// error(i, mn, num, dist);
			num -= mn;
			if (arr[*it] == 0) se.erase(it);			
			ans += mn * dist;
			it = se.find_by_order(query[i].f1);
		}
    	println(ans);
    }
