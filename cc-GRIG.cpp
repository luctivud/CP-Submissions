...
void solveEachTest(lld _TestCase = 1) {
	...
	while (sz > 0) {
		lld thisSubarrayParity = 0ll;
		map<char, lld> cnt;
		cnt.clear();
		for4(i, 0ll, sz, 1) {
			cnt[s[i]] += 1;
			// deb2(s[i]); deb1(cnt[s[i]]);
			if (cnt[s[i]] & 1) {
				thisSubarrayParity += 1;
			} else {
				thisSubarrayParity -= 1;
			}

		}
		if (thisSubarrayParity <= 1) {
			ans = max(ans, sz);
		}
		// error(sz, thisSubarrayParity);
		...
		for4(i, sz, n, 1) {
			...
			// error(i, sz, thisSubarrayParity, ans);
		}
		// error(sz, thisSubarrayParity);
		if (ans >= sz) {
			break;
