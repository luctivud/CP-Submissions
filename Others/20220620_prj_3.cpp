...
class Tracker {
    map<string, set<int>> trackerMap;
private:
    void initiate(string s) {
        for (int i = 1; i < 1000; i++) {
            trackerMap[s].insert(i);
        }
    }
public:
    string allocate(string s) {
        string name = s;
        int num = 0;
        if (trackerMap.find(name) == trackerMap.end()) {
            initiate(name);
        }
        num = *(trackerMap[name].begin());
        trackerMap[name].erase(trackerMap[name].begin());
