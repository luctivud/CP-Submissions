...
void solveEachTest(int _TestCase) {
    ...
    lld sz = (lld)posOfM.size();

    lld maxLen = 1, thisLen = 1;

    forn1(i, sz-1) {
        if (posOfM[i] == posOfM[i-1] + 1) {
            thisLen += 1;
        } else {
            thisLen = 1;
        }
    ...
    // forn(i, sz) {
    //     _____error_____(posOfM[i], posOfM[posMid] + (i - posMid));
    //     ans += !!(llabs(posOfM[i] - (posOfM[posMid] + (i - posMid))));
    // }
