...
void solveEachTest(lld _TestCase) {
	// cout << "Case#" << _TestCase << ": ";
	lld n, m, x1, y1, x2, y2; read(n, m, x1, y1, x2, y2);
	...
	lld ans = Binomial(n + m, n, MOD);

	ans -= (Binomial(x1 + y1, x1, MOD) * Binomial(n - x1 + m - y1, n - x1, MOD)) % MOD;
	ans += MOD;
	ans %= MOD;
	ans -= (Binomial(x2 + y2, x2, MOD) * Binomial(n - x2 + m - y2, n - x2, MOD)) % MOD;
	ans += MOD;
	ans %= mod;

	if (x1 <= x2 and y1 <= y2) {
		ans += ((Binomial(x1 + y1, x1, MOD) * Binomial(x2 - x1 + y2 - y1, x2 - x1, MOD)) % MOD * Binomial(n - x2 + m - y2, n - x2, MOD)) % MOD;
	}
