...
lld solve(lld num, lld n) {
    // _____error_____(num);
    // _____error_____(dp);

    if (findMaxDigit(num) < 2) {
        return dp[num] = -1;
    }
    
    string s_num = to_string(num);

    if ((lld)s_num.size() == n) return 0;
    if ((lld)s_num.size() > n) return dp[num] = -1;
...
void solveEachTest(int _TestCase) {
    ...
    // lld res = 0;
    // while (findLength(num) < n) {
    //     lld mx = findMaxDigit(num);
    //     if (mx < 2) {
    //         res = -1;
    //         break;
    //     }
