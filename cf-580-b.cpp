...
void solveEachTest(lld _TestCase = 1) {
	...
	lld i = 0, j = 0;
	lld maxFriendship = 0ll;
	while (j < n and frnds[j].first - frnds[i].first < d) {
		maxFriendship += frnds[j++].second;
	}
	lld ans = maxFriendship;
	while (j < n) {
		maxFriendship -= frnds[i++].second;
		while (j < n and frnds[j].first - frnds[i].first < d) {
			maxFriendship += frnds[j++].second;
		}

		ans = max(ans, maxFriendship);
	}
