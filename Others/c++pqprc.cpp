...
class cmp {
public:
    bool operator() (const pair<lld, pair<lld, lld>> &f, const pair<lld, pair<lld, lld>> &s) {
        return (f.s2.f1 * f.s2.s2 == s.s2.f1 * s.s2.s2) ? (f.s2.s2 == s.s2.s2) ? (f.f1 > s.f1) : (f.s2.s2 < s.s2.s2) : (f.s2.f1 * f.s2.s2 < s.s2.f1 * s.s2.s2);
    }
};  
...
void solveEachTest(lld _TestCase = 1) {
    ...
    priority_queue<pair<lld, pair<lld, lld>>, vector<pair<lld, pair<lld, lld>>>, cmp> pq;
    for4(i, 0, n, 1) {
        pq.push({i+1, {arr[i].f1, arr[i].s2}});
    }
    // cout << "\n"; 
