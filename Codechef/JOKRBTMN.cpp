...
public:
    void SolveEachTest(int _TestCase) {
        ...
        forn(i, m) {
            ...
            forn(j, k-1) {
                lld x; cin >> x;
                Union(x, bb, par3t, ran4k);
            }
        }

        // set<lld> uniqueColors;
        vector<lld> colors;
        forn(i, l) {
            lld el; cin >> el;
            // uniqueColors.insert(find_repr(el, par3t, ran4k));
            colors.push_back(find_repr(el, par3t, ran4k));
        }

        _____error_____(colors);

        lld ans = unique(colors.begin(), colors.end()) - colors.begin();
        cout << ans;
    }
