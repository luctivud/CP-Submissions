...
void dfs(vector<lld> adj[], lld node, lld d, vector<bool> &visited, vector<lld> &depth) {
	// error(node, d);
	visited[node] = true;
	depth[node] = d;
	EACH(n, adj[node]) {
		if (!visited[n]) {
			dfs(adj, n, d + 1, visited, depth);
		}
	}
	return;
}
...
void solveEachTest(lld _TestCase) {
	// cout << "Case#" << _TestCase << ": ";
	lld n, a, b, da, db; read(n, a, b, da, db);
	...
	// dfs
	dfs(adj, b, 0, visited, depth);
	lld node = (lld)(max_element(1 + all(depth)) - depth.begin());
	// error(node);
	// debspace(depth);
	lld initialDistance = depth[a];
	fill(all(depth), 0ll);
	fill(all(visited), false);
	// dfs
	dfs(adj, node, 0, visited, depth);
	lld diameter = *max_element(1 + all(depth));

	// error(initialDistance, diameter);
	bool ok = false;
	if (((initialDistance <= da) or (da >= ((diameter + 1) / 2))) or (db <= 2 * da)) {
		ok = true;
	}

	println(ok ? "Alice" : "Bob");
