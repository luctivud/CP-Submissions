...
void dfs(lld node, lld par, vector<lld> &depth, vector<lld> adj[]) {
	depth[node] = depth[par] + 1;
	EACH(i, adj[node]) {
		if (i != par) {
			dfs(i, node, depth, adj);
		}
	}
}
...
void solveEachTest(lld _TestCase) {
	...
	vector<lld> depth(n + 1, -1ll);

	dfs(1, 0, depth, adj);

	// println(depth);

	lld node = max_element(1 + all(depth)) - depth.begin();

	fill(all(depth), -1ll);
	dfs(node, 0, depth, adj);
	...
	vector<lld> ans(n + 1, 0ll);

	forn1(i, n) {
		if (depth[i] + 1 > diameter) {
			ans[i] = (diameter + 1);
		} else {
			ans[i] = (diameter);
		}
	...
	node = max_element(1 + all(depth)) - depth.begin();
