...
void traverse
(
    lld i, lld j, vector<vector<char>> grid, set<pair <lld, lld>> &visited,
    lld up = 0ll, lld down = 0ll, lld left = 0ll, lld right = 0ll
)
{
	if (!isvalid(i, j)) return;
	cnt++;
	visited.insert({i, j});
	if (i - 1 < 0 or grid[i - 1][j] != '#') up += 1;
	if (i + 1 >= n or grid[i + 1][j] != '#') down += 1;
	if (j - 1 < 0 or grid[i][j - 1] != '#') left += 1;
	if (j + 1 >= m or grid[i][j + 1] != '#') right += 1;
	// error(i, j, up, down, left, right);
	mx = max({mx, left, right, up, down});
	forn(k, 4) {
		lld newI = i + d4i[k];
		lld newJ = j + d4j[k];
		if (isvalid(newI, newJ) and (visited.find({newI, newJ}) == visited.end()) and grid[newI][newJ] == '#') {
			// traverse(newI, newJ, grid, visited, ((d4i[k] == 0) ? 0ll : up), ((d4i[k] == 0) ? 0ll : down), ((d4j[k] == 0) ? 0ll : left), ((d4j[k] == 0) ? 0ll : right));
			if (d4i[k] == 0) {
				traverse(newI, newJ, grid, visited, up, down, 0ll, 0ll);
			}

			if (d4j[k] == 0) {
				traverse(newI, newJ, grid, visited, 0ll, 0ll, left, right);
			}

			next_j = max(next_j, newJ+1);
			next_i = max(next_i, newI+1);
		}
	}
}
...
void solveEachTest(lld _TestCase) {
	...
	mx = 0ll;
	cnt = 0ll;
	vector<vector<char>> matr(n, vector<char>(m)) ;	read(matr);

	// println(matr);

	// vector <vector<bool>> visited(n, vector<bool> (m, false));
	set<pair<lld, lld>> visited;
	...
	// 		next_j = max(next_j, j);
	// 		// error(i, j);
	// 		if (visited.find({i, j}) == visited.end() and i < n and j < m and matr[i][j] == '#') {
	// 			traverse(i, j, matr, visited);
	// 		}
	// 		if (i == next_i) i++; else i = next_i+1;
	// 		if (j == next_j) j++; else j = next_j+1;
	// 	}
