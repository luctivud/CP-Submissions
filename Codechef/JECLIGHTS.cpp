...
void solveEachTest(int _TestCase) {
    ...
    vector<string> arr(n);
    vector<vector<lld>> dp(n, vector<lld> (2, 0));

    forn(i, n) {
        cin >> arr[n-i-1];
    }

    vector<array<lld, 2>> LR(n);
    forn(i, n) {
        LR[i][0] = m-1, LR[i][1] = 0;
    ...
    NonIterable.print(LR);

    for (lld i = 1; i < n; i++) {
        if (i == n-1) {
            dp[i][0] = 1 + min(dp[i-1][0] + LR[i][1], dp[i-1][1] + m-1);
            dp[i][1] = 1 + min(dp[i-1][1] + m - LR[i][0] - 1, dp[i-1][1] + m-1);       
            break;
        }
        dp[i][0] = 1 + min(dp[i-1][0] + 2 * LR[i][1], dp[i-1][1] + m-1);
        dp[i][1] = 1 + min(dp[i-1][1] + 2 * (m - LR[i][0] - 1), dp[i-1][1] + m-1);
    }

    NonIterable.print(dp);
    lld ans = min(dp[n-1][0], dp[n-1][1]);
    while (n > 0 && LR[n-1][1] == 0 and LR[n-1][0] == m-1) {
        n -= 1;
        ans -= 1;
    }
    cout << ans;
