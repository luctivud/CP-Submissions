...
void solveEachTest(int _TestCase) {
    ...
    while (q.size()) {
    	...
    	q.pop();
    	for (auto i : adj[node.f1]) {
			// error(i, depth[i], node.s2+1);
    		if (depth[i] > (node.s2 + 1)) {
    			depth[i] = node.s2 + 1;
    			q.push(make_pair(i, node.s2 + 1));
    		}
    	}
    }

    lld mxDep = 0, mxNode = 0;
    forn(i, n) {
    	if (depth[i] == INFi) depth[i] = -1;
    	if (depth[i] > mxDep) {
    		mxDep = depth[i];
    		mxNode = i;
    	}
