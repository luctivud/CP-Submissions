...
public:
    ...
    Node (lld m_cnt, lld m_lVal, lld m_rVal, lld m_lSize, lld m_rSize, bool m_connected) : 
        cnt(m_cnt), lVal(m_lVal), rVal(m_rVal), lSize(m_lSize), rSize(m_rSize), connected(m_connected) {
    }
};
...
Node merge_func(Node l, Node r) {
    lld res = l.cnt + r.cnt;
    lld lVal_new, rVal_new, lSize_new, rSize_new;
    bool connected_new = (l.connected & r.connected);
    ...
    if (l.rVal <= r.lVal) {
        res += (l.rSize * r.lSize);
        if (l.connected) {
            lSize_new = l.lSize + r.lSize;
        }
        if (r.connected) {
            rSize_new = l.rSize + r.lSize;
        }
    } else {
        connected_new = false;
    }
    ...
    if (connected_new) {
        lSize_new = l.rSize + r.lSize;
        rSize_new = l.rSize + r.lSize;
    }

    return Node(res, lVal_new, rVal_new, lSize_new, rSize_new, connected_new);
}
...
void solveEachTest(int _TestCase) {
    ...
    forn(i, n) {
        lld temp; cin >> temp;
        arr[i] = Node(1, temp, temp, 1, 1, true);
    }

    segment_tree segg(arr, merge_func, Node());
    ...
    forn(qq, q) {
        ...
        if (qType == 1) {
            ...
            cin >> pos >> val;
            pos--;
            segg.set(pos, Node(1, val, val, 1, 1, true));
        } else {
            int l, r; cin >> l >> r;
            cout << segg.query(l-1, r).cnt << "\n";
        }
    }
