...
void solveEachTest(lld _TestCase) {
	// cout << "Case#" << _TestCase << ": ";
	lld n, m; read(n, m);
	vector<lld> depth(n); read(depth);

	vector<lld> NodesAtDepth[n + 1];

	forn(i, n) {
		NodesAtDepth[depth[i]].pb(i + 1);
	}

	lld prevNode = 1;
	lld ends = n + 1;
	forn1(i, n) {
		if (len(NodesAtDepth[i]) == 0) {
			ends = i;
			break;
		}
		EACH(it, NodesAtDepth[i]) {
			println(prevNode, it);
			m -= 1;
		}
		prevNode = NodesAtDepth[i][0];
	}

	lld hairu = 1ll;
	while (m > 0 and hairu <= ends) {
		lld sz = len(NodesAtDepth[hairu]);
		forn(i, sz) {
			for4(j, i + 1, sz, 1) {
				// error(hairu, i, j);
				println(NodesAtDepth[hairu][i], NodesAtDepth[hairu][j]);
				m -= 1;
				if (m == 0) return;
			}
		}
	...
	while (m > 0 and hairu < ends) {
		lld sz = len(NodesAtDepth[hairu]);
		lld sz2 = len(NodesAtDepth[hairu + 1]);
		forn1(i, sz - 1) {
			forn(j, sz2) {
				println(NodesAtDepth[hairu][i], NodesAtDepth[hairu + 1][j]);
				m -= 1;
				if (m == 0) return;
			}
		}
