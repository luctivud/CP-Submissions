...
lld findSq(lld n) {
	...
	g = __gcd(n3, div);
	n3 /= g; div /= g;
	return (((n1 % MOD) * (n2 % mod)) % mod * (n3 % mod)) % mod;
}
...
void solveEachTest(int _TestCase) {
	...
	lld res1 = (mod + findSum(r) - findSum(l-1)) % MOD;

	lld res2 = (mod + findSq(r) - findSq(l-1)) % mod;

	println((((res1 * res1) % MOD) - res2) % MOD);
