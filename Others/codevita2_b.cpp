...
void solveEachTest(lld _TestCase) {
    ...
    vector<lld> collectionOfRepr(n, 0), sizes(n, 0);

    lld pairs; read(pairs);
    ...
    forn(i, n) {
    	lld repr = find_repr(i, parent, rank);
    	collectionOfRepr[repr] += arr[i];
    	sizes[repr] += 1;
    }


    lld ans = -1, minSz = n+1, repres = 0;
    forn(i, n) {
    	if (collectionOfRepr[i] > ans) {
    		ans = collectionOfRepr[i];
    		repres = i;
    		minSz = sizes[i];
    	} else if ((collectionOfRepr[i] == ans) and (sizes[i] < minSz)) {
    		repres = i;
    		minSz = sizes[i];
    	}
    }

    // println(collectionOfRepr);
    // println(sizes);
    // error(repres);

    forn(i, n) {
    	// error(find_repr(i, parent, rank));
    	if (find_repr(i, parent, rank) == repres) {
    		print(i+1, "");
    	}
    }
