...
void solveEachTest(int _TestCase) {
    ...
    vector<vector<lld>> dist(26, vector<lld> (26, inf));
    ...
    forn(i, 26) {
        ...
        while (q.size()) {
            ...
            for (auto neigh : adj[u]) {
                if (dist[i][neigh] > dist[i][u] + 1) {
                    dist[i][neigh] = dist[i][u] + 1;
                    q.push(neigh);
                }
            }
        }
    ...
    forn(i, 26) {
        lld res = 0ll;
        forn(j, 26) {
            // _____error_____(i, j, dist[j][i]);
            if (cnt[j] > 0 and dist[j][i] == inf) {
                res = inf;
                break;
            }
            res += cnt[j] * dist[j][i];
