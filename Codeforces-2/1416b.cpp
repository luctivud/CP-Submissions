...
void solveEachTest(int _TestCase) {
    ...
    for (lld i = 2; i <= n; i++) {
        if (arr[i] % i == 0) {
            ans.push_back({i, 1, arr[i] / i});
            arr[1] += arr[i];
            arr[i] = 0;
        } else if (arr[1] >= (i - arr[i] % i)) {
            ans.push_back({1, i, i - (arr[i] % i)});
            ans.push_back({i, 1, (arr[i] / i)+1});
            arr[1] += arr[i];
            arr[i] = 0;
        }
    }
    ...
    for (lld i = 2; i <= n; i++) {
        ans.push_back({1, i, sum_of_arr / n});
    }

    cout << ans.size() << "\n";
    for (auto [xi, xj, xx] : ans) {
        brr[xi] -= xi * xx;
        brr[xj] += xi * xx;
        cout << xi << " " << xj <<  " " << xx << "\n";
    }
