...
void solveEachTest(lld _TestCase = 1) {
	...
	string ansName = "No Research This Month";
	lld maxCC = 0;
	forn(tt, n) {
		...
		lld thisMaxCC = 1ll;
		forn(i, 26) {
			if (thisCount[i] > mainCount[i]) {
				thisMaxCC = 0ll;
				break;
			}

			if (mainCount[i] == 0 or thisCount[i] == 0) continue;
			// error(tt, mainCount[i], thisCount[i], i);
			thisMaxCC *= choose(mainCount[i], thisCount[i]);
			thisMaxCC %= mod;
		}
		thisMaxCC %= mod;
		println(thisMaxCC);
...
signed main() {
	ios_base::sync_with_stdio(false); cin.tie(nullptr); cout.precision(10); cout << fixed;
