...
// void dfs(lld node, lld par, vector<lld> &depth, vector<lld> adj[], vector<lld> candies, vector<bool> &visited) {
// 	visited[node] = true;
// 	depth[node] = depth[par] + 1;
// 	EACH(i, adj[node]) {
// 		if (i != par and candies[i] and !visited[i]) {
// 			dfs(i, node, depth, adj, candies, visited);
// 		}
// 	}
// }
...
class cmp {
public:
	bool operator() (const pair <lld, lld> &a, const pair <lld, lld> &b) {
		return a.f1 < b.f1;
	}
};
...
void solveEachTest(lld _TestCase) {
	...
	vector<lld> depth(n + 1, INFll / 2);
	// vector<bool> visited(n + 1, false);

	// dfs(a, 0, depth, adj, candies, visited);
	...
	while (!pq.empty()) {
		pair<lld, lld> node = pq.top();
		pq.pop();
		for (auto i : adj[node.s2]) {
			if (candies[i] and depth[i] > depth[node.s2] + 1) {
				depth[i] = depth[node.s2] + 1;
				pq.push({depth[i], i});
			}
		}
	}
	...
	println((depth[b] == INFll / 2) ? -1 : depth[b]);
