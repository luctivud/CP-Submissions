...
lld findDiff(lld a, lld b, lld n) {
	if (a > b) swap(a, b);
	return min(b - a, n - b + a);
}

void Search(lld l, lld r, vector<lld> arr) {
	if (l > r) return;
	lld mid = l + (r - l) / 2;
	lld sel = mid % n;
	if (sel == 0) sel = n;
	...
	lld n1 = 0LL, n2 = 0ll, n3 = 0ll;
	forn(i, w) {
		n1 += findDiff(sel1, arr[i], n);
		n2 += findDiff(sel, arr[i], n);
		n3 += findDiff(sel2, arr[i], n);
	}
	// error(l, r, mid, sel, sel1, sel2, n1, n2, n3);
	// error(n1, n2, n3);
