...
void solveEachTest(int _TestCase) {
	...
	// forn(forbidden, n) {
	{
		for (auto start : starts) {
			lld skipped = -1;
			bool skippedtbh = false;
			lld newEndA = 0ll;// offset + n
			lld currPosA = start;
			lld currPosB = 0ll;
			newEndA = currPosA + n;
			currPosB += 1;
			while (currPosB < n and currPosA < (newEndA + 1)) {
				while (currPosA <= newEndA && (a[currPosA%n] != b[currPosB])) {
					if (skipped == currPosA-1)
						skippedtbh = true;
					skipped = currPosA;
					currPosA += 1;
				}
				if (currPosA == newEndA+1) break;
				currPosB += 1;
			}
			if (currPosB == n) {
				if (((currPosA != newEndA-1 && currPosA != newEndA) || skippedtbh))
					ok = true;
				// debugluc(skipped, currPosA);
				break;
			}
		}
		// if (ok) break;
