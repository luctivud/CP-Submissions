...
void solveEachTest(lld _TestCase) {
	...
	forn1(i, n) {
		dp[i][0] = (arr[i - 1] & 1) ? max(dp[i - 1][1] + arr[i - 1], dp[i - 1][0]) : max({dp[i - 1][0] + arr[i - 1], dp[i - 1][0], arr[i - 1]});
		dp[i][1] = (arr[i - 1] & 1) ? max({dp[i - 1][0] + arr[i - 1], arr[i - 1], (dp[i - 1][1] & 1) ? dp[i - 1][1] : arr[i - 1]}) : ((dp[i - 1][1] + arr[i - 1]) & 1) ? (dp[i - 1][1] + arr[i - 1]) : 0;
		// error(dp[i][0], dp[i][1]);
	}
