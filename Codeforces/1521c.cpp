...
void askQuery(lld t, lld i, lld j, lld x) {
	cout << "?" << " " << t << " " << i << " " << j << " " << x << "\n";
}
...
void solveEachTest(int _TestCase) {
    ...
    while (i+2 < n) {
    	lld elem;
    	set<lld> part1, part2;
    	askQuery(1, i+1, i+2, n-1);
    	cin >> elem;
    	part1.insert(elem);
    	askQuery(2, i+1, i+2, 1);
    	cin >> elem;
    	part1.insert(elem);
    	askQuery(1, i+2, i+3, n-1);
    	cin >> elem;
    	part2.insert(elem);
    	askQuery(2, i+2, i+3, 1);
    	cin >> elem;
    	part2.insert(elem);

    	auto it = part1.begin();
    	lld res = 0ll;
    	if (part2.find(*it) != part2.end()) {
    		res = *it;
    		part1.erase(it);
    		part2.erase(part2.find(res));
    	} else if (part2.find(*(++it)) != part2.end()) {
    		res = *it;
    		part1.erase(it);
    		part2.erase(part2.find(res));
    	}
