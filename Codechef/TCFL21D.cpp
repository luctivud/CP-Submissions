...
void solveEachTest(int _TestCase) {
    ...
    lld ans = 0ll;
    forn(i, n-2) {
    	lld startingIndex = i, endingIndex = i+2;
    	set<lld> indices;
    	for4(j, startingIndex, endingIndex+1, 1) {
    		indices.insert(j);
    	}

    	lld minIndex = min_element(arr+startingIndex, arr+endingIndex+1) - arr;
    	indices.erase(minIndex);
    	lld maxIndex = (arr[*indices.begin()] > arr[*indices.rbegin()] ? *indices.begin() : *indices.rbegin());
    	indices.erase(maxIndex);
    	lld midIndex = *indices.begin();
    	lld diff = (arr[maxIndex]+1) - (arr[minIndex] + arr[midIndex]);
    	diff = max(diff, 0ll);
    	if (arr[minIndex] == arr[midIndex] and arr[midIndex] == arr[maxIndex]) {
    		if (minIndex < midIndex) {
    			swap(minIndex, midIndex);
    		}
    	...
    	if (diff) {
    		lld smallDiff = diff / 2;
    		lld largeDiff = diff - smallDiff;
    		if ((smallDiff == 0) and i > 0 and arr[i-1] == 1) {
    			arr[i-1]++;
    			ans++;
    		}
    		arr[midIndex] += smallDiff;
    		arr[minIndex] += largeDiff;
    		ans += diff;
    	}
