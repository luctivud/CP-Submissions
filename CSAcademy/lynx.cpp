...
void solveEachTest(int _TestCase) {
    ...
    pair<lld, lld> arr[n];
    lld prefix_sum[n+1] = {0};
    forn(i, n) Read(arr[i].f1), arr[i].s2 = i;

    sort(arr, arr+n);
    forn(i, n) {
    	prefix_sum[i+1] = prefix_sum[i] + arr[i].f1;
    }

    vector<lld> ans(n, 0);
    forn(i, n) {
    	lld num = arr[i].f1, ind = arr[i].s2;
    	lld l = i+1, r = n-1;
    	lld res = 1ll;
    	if (i>0 and arr[i].f1 == arr[i-1].f1) {
    		ans[ind] = ans[arr[i-1].s2];
    		continue;
    	}
    	while (l <= r) {
    		lld mid = l + (r - l) / 2;
    		lld sum = prefix_sum[mid+1] - prefix_sum[i];
    		lld total_num = mid - i + 1;
    		sum -= (total_num * num);
    		if (sum <= k) {
    			l = mid + 1;
    			res = max(res, total_num);
    		} else {
    			r = mid - 1;
    		}
    		// error(i, mid, num, sum, prefix_sum[mid+1], prefix_sum[i], total_num);
    	}
    	ans[ind] = res;
    }
