...
void solveEachTest(int _TestCase) {
    ...
    cin >> n;
    string a; cin >> a;
    vector<lld> doneNum(n+1, -1);
    string b(a);
    forn(i, n+1) {
        bool ok = true;
        forn(j, n) {
            // random positions will help ig
            lld pos = (rand() % n);
            while (doneNum[pos] == i) {
                pos = (rand() % n);
            }
            doneNum[pos] = i;
            // _____error_____(i, pos);
            lld indA = getAInd(i, pos);
            lld indB = getBInd(i, pos);
            // _____error_____(indA, indB);
            // _____error_____(a[indA], b[indB]);
            if (a[indA] != b[indB]) {
                ok = false;
                break;
            }
