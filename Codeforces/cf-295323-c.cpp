...
const long long MAXN = (lld)(100) + 1ll; // MAXN Size
...
void solveEachTest(lld _TestCase) {
	...
	// set <lld> visited_factors;

	vector<lld> visited_factors(MAXN + 5, false);

	forn(i, n) {
		lld val = arr[i];
		while (val > 1) {
			visited_factors[smallest_prime_factor[val]] += 1;
			lld hairu = smallest_prime_factor[val];
			while (smallest_prime_factor[val] == hairu) {
				val /= smallest_prime_factor[val];
			}
		}
	}
	...
	forn(i, MAXN) {
		if (visited_factors[i] == n - 1) {
			ok = true;
		}
	}

	// debhairu(visited_factors);
	// println(ok);
