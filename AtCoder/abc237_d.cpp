...
public:
	...
	DLLNode(lld a, DLLNode*l, DLLNode*r) 
	{
		value = a;
		left = l;
		right = r;
...
void solveEachTest(int case_t) 
{
    ...
    DLLNode *node = new DLLNode(0ll, NULL, NULL);
    DLLNode *head = node;
    DLLNode *prev = node;
    ...
    for (lld i = 0; i < n; i++) 
    {
    	DLLNode *curr = new DLLNode(0ll, NULL, NULL);
    	(*curr).value = i+1;
    	if (s[i] == 'L') 
    	{
    		DLLNode *temp = (*prev).left;
    		(*prev).left = curr;
    		(*curr).right = prev;
    		if (temp != NULL)
    			(*temp).right = curr, (*curr).left = temp;	
    		else
    			head = curr;
    	} 
    	else 
    	{
    		DLLNode *temp = (*prev).right;
    		(*prev).right = curr;
    		(*curr).left = prev;
    		if (temp != NULL)
