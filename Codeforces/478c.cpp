...
#ifdef LUCTIVUD
	freopen("/home/luctivud/Streams/IO/input.txt", "r", stdin);
	freopen("/home/luctivud/Streams/IO/output.txt", "w", stdout);
#endif
...
void offset(vector<lld> &arr, lld o) {
	EACH(it, arr) {
		it += o;
	}
}
...
void solveEachTest(lld _TestCase) {
	// cout << "Case#" << _TestCase << ": ";
	vector<lld> arr(3); read(arr);

	// lld mn = *min_element(all(arr));
	...
	// mn = min({arr[1], arr[2], (arr[1] + arr[2]) / 3});

	// println(mn + ans);


	println(max(min(arr[0] + arr[1], arr[3]), min({arr[0], arr[1], arr[2]})));
