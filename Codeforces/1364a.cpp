...
void solveEachTest(lld _TestCase) {
	...
	lld firstOne = n, lastOne = -1ll;
	forn(i, n) {
		arr[i] %= x;
		firstOne += (arr[i] != 0 and firstOne == n) ? -n + i : 0;
		lastOne = (arr[i] != 0) ? i : lastOne;
		sum += arr[i];
		sum %= x;
	}


	// error(sum, firstOne, lastOne);

	println(sum % x != 0 ? n : max(n - firstOne - 1, lastOne));
