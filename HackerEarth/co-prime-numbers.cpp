...
void solveEachTest(int _TestCase) {
    ...
    for (lld i = 2; i <= (lld)sqrt(n); i++) {
        lld p = i;
        // set<lld> not_ans;
        lld lb = i;
        lld ub = (n / i);
        // lld lb = ((i / sp) + 1) * sp;
        vector<lld> prime_facs;
        while (p > 1) {
            lld sp = smallest_prime_factor[p];
            while (p % sp == 0) {
                p /= sp;
            }
            prime_facs.push_back(sp);
            // _____error_____(i, sp, lb, ub);
            // for (lld j = lb; j <= ub; j += sp) {
            //     not_ans.insert(j);
            // }
        ...
        lld res = 0ll;
        lld sz = (lld)prime_facs.size();
        for (lld taken = 1ll; taken <= sz; taken++) {
            vector<lld> perm(sz, 0);
            for (lld ss = 0; ss < taken; ss++) {
                perm[sz-ss-1] = 1;
            }
            ...
            do {
                lld num = 1ll;
                forn(j, sz) {
                    num *= perm[j] ? prime_facs[j] : 1ll;
                }
                lld yestart = ((lb + num - 1) / num);
                lld yestop = (ub / num);
                res += (taken & 1 ? 1 : -1) * ((ub - lb + 1) - (yestop - yestart + 1));
            } while (next_permutation(all(perm)));
        }

        ans += res;

        // _____error_____(not_ans);
        // ans += (ub - lb + 1) - (lld)not_ans.size();
    }
