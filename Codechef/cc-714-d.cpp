...
void solveEachTest(lld _TestCase = 1) {
	...
	lld a, b, c, d;
	a = 1, b = n, c = 1, d = n;
	lld al = a, ar = n, bl = 0, br = n, cl = 0, cr = n, dl = 0, dr = n;
	lld numberAndar;
	while (a < b and c < d) {
		lld am = al + (ar - al) / 2, bm = bl + (br - bl) / 2;
		lld cm = cl + (cr - cl) / 2, dm = dl + (dr - dl) / 2;

		println("?", am, bm, cm, dm);
		read(numberAndar);
		if (numberAndar == 2) {
			a = am, b = bm, c = cm, d = dm;
			ar = am - 1, bl = bm + 1, cr = cm - 1, dl = dm + 1;
			continue;
		} else {
			println("?", am, bm, c, d);
			read(numberAndar);
			if (numberAndar == 2) {
				a = am, b = bm, c = c, d = d;
				ar = am - 1, bl = bm + 1, cl = cm + 1, dr = dm - 1;
				continue;
				// } else if (numberAndar == 1) {
			...
			if (numberAndar == 2) {
				a = a, b = b, c = cm, d = dm;
				al = am + 1, br = bm - 1, cr = cm - 1, dl = dm + 1;
				continue;
				// } else if (numberAndar == 1) {
