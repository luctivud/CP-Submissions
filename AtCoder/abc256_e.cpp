...
void solveEachTest(int _TestCase) {
    // cout << "Case #" << _TestCase << ": ";
    lld n; cin >> n;
    vector<lld> visited(n+1, 0ll);
    vector<lld> p(n+1, 0ll), x(n+1, 0ll);
    forn1(i, n) {
        cin >> p[i];
    }
    forn1(i, n) {
    ...
    forn1(i, n) {
        ...
        lld node = p[i];
        set<lld> edgesInComponent;
        edgesInComponent.insert(x[i]);
        visited[i] = i;
        while (!visited[node]) {
            edgesInComponent.insert(x[node]);
            visited[node] = i;
            node = p[node];
        }
        if (visited[node] == i) {
            lld node2 = i;
            while (node2 != node) {
                edgesInComponent.erase(edgesInComponent.find(x[node2]));
                node2 = p[node2];
            }
            ans += *(edgesInComponent.begin());
        }
    }
