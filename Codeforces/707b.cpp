...
void solveEachTest(int _TestCase) {
    // cout << "Case #" << _TestCase << ": ";
    lld n, m, k; cin >> n >> m >> k;
    vector<array<lld, 3>> edges;

    forn(i, m) {
    	lld u, v, w; cin >> u >> v >> w;
    	edges.push_back((array<lld, 3>){u, v, w});
    }

    bitset<100001>special_cities;

    forn(i, k) {
    	cin >> TempVar;
    	special_cities[TempVar] = 1;
    }

    lld ans = lld(1e12);

    for (auto [u, v, w] : edges) {
    	if (special_cities[u] & special_cities[v]) {
    		continue;
    	} else if (special_cities[u] | special_cities[v]) {
    		ans = min(ans, w);
    	}
    }

    cout << ((ans == lld(1e12)) ? -1 : ans) << "\n";
