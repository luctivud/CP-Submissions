...
void brute (lld n, lld k, string s) {
    sort(all(s));
    map<lld, vector<string>> res;
    do {
        lld ans = 0ll;
        forn(x, n-1) {
            ans += (10 * (s[x] - '0')) + (s[x+1] - '0');
        }
        res[ans].push_back(s);
    } while (next_permutation(all(s)));

    _____error_____(*(res.begin()));
    NonIterable.print(res);
}


void solve(lld n, lld k, string &s) {
    lld i = 0, j = 0;
    ...
    for (lld x = 0; x < k and j < n and j > 0; x++) {
        swap(s[j], s[j-1]);
        j -= 1;
    }
    return;
