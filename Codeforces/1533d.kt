...
fun solve(_TestCase: Int) {
    ...
    val (n, m) = nextInts()
    var cntElem = HashMap<String, Int>()
    for (i in 1..n) {
        var s = next()
        val c = cntElem.getOrElse(s) { 0 }
        cntElem.put(s, c+1)
    }
    // println("cntElem is $cntElem")
    val Q = nextInt()
    for (qq in 1..Q) {
        val s = next()
        var ans: Int = 0
        ...
        for (i in 0..m) {
            var newS = s.substring(0, i) + s.substring(i+1, m+1)
            se.add(newS)
            // println("newS is $newS")
        }
        // println("se is $se")
        for (el in se) {
            ans += cntElem.getOrElse(el) {0}
        }
        println(ans)
    }
}
