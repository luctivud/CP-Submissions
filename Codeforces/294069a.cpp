...
void solveEachTest(lld _TestCase) {
	...
	cnt[0] = 0;
	forn(i, n) {
		lld temp = prefix_sum[i + 1] % k;
		// error(temp);
		if (cnt[temp] == 0 and temp != 0) {
			cnt[temp] = i + 1;
		} else if (prefix_sum[i + 1] - prefix_sum[cnt[temp]] < 0) {
			cnt[temp] = i + 1;
		}
		ans = max(ans, prefix_sum[i + 1] - prefix_sum[cnt[temp]]);
		// error(temp, cnt[temp], ans);
	}
