...
void solveEachTest(int _TestCase) {
    ...
    while (pq.size() > 1 and k > 0) {
        ...
        lld toBeUsed = min(val1 - val2, k / cnt1);
        _____error_____(toBeUsed);
        k -= toBeUsed * cnt1;
        ans += cnt1 * (die(val1) - die(val1 - toBeUsed));
        if (toBeUsed == 0) {
            ans += (k * val1);
            k = 0;
        }
        if (val1 - toBeUsed > val2) {
            pq.push({val1 - toBeUsed, cnt1});
            pq.push({val2, cnt1});
        } else {
            pq.push({val2, cnt2 + cnt1});
        }
        _____error_____(val1, cnt1);
        _____error_____(val2, cnt2);
        _____error_____(k, ans);
        // NonIterable.print(pq);
    }
