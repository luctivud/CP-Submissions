...
vector<int> totalNumbers(int n, int k, vector<int> a) {
	vector<int> ans = {1};
	set<int> pnums, toRemove;
	for (int i = 0; i < k; i++) {
		if (prime_numbers[i] > k) break;
		pnums.insert(prime_numbers[i]);
	}
	for (auto i : a) {
		while (i > 1) {
			int j = smallest_prime_factor[i];
			while (i % j == 0) {
				i /= j;
			}
			if (pnums.find(j) != pnums.end()) {
				pnums.erase(j);
				toRemove.insert(j);
			}
		}
	...
	for (auto i : toRemove) {
		for (int j = i; j <= k; j += i) {
			if (couldBeAns.find(j) != couldBeAns.end()) {
				couldBeAns.erase(j);
			}
		}
	}
	for (auto i : couldBeAns) {
		ans.push_back(i);
	}
