...
void solveEachTest(int _TestCase) {
    ...
    while (q.size()) {
        // NonIterable.print(q);
        auto [sz, wait, u] = *q.begin();
        q.erase(q.begin());
        for (auto [nig, w] : adj[u]) {
            if (wait == depths[u] and nig != u and depths[nig] > wait + (w * sz)) {
                depths[nig] = wait + (w * sz);
                q.insert({sz+1, depths[nig], nig});
            }
        }
    }

    forn1(i, n) {
        cout << (depths[i] == inf ? -1 : depths[i]) << "\n";
    }
