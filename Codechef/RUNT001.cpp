...
void solveEachTest(lld _TestCase) {
    // cout << "Case #" << _TestCase << ": ";
    lld floors, rooms, k; read(floors, rooms, k);
    vector<pair<lld,lld>> affxnaff;
    forn(i, floors) {
    	read(_TestCase);
    	affxnaff.pb(make_pair(rooms - _TestCase, _TestCase));
    }

    sort(all(affxnaff), [] (pair<lld, lld> &a, pair<lld, lld> &b) {
    	return a.f1 > b.f1;
    });
