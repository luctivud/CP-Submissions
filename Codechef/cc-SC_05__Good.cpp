...
lld solve(lld k, lld coin, lld a, lld b, vector<vector<lld>> &dp) {
	if (k < 0) return 0;
	if (k == 0) return coin;
	lld &ans = dp[k][coin];
	if (ans != -1) return ans;
	if (coin >= a) {
		return (ans = max(solve(k - 2, coin - a + b, a, b, dp), solve(k - 1, coin + 1, a, b, dp)));
	}

	return ans = solve(k - 1, coin + 1, a, b, dp);
...
void solveEachTest(lld _TestCase) {
	// cout << "Case#" << _TestCase << ": ";
	lld k, a, b; read(k, a, b);
	vector<vector<lld>> dp(k + 1, vector<lld>(100001, -1)) ;

	println(solve(k, 1ll, a, b, dp));
