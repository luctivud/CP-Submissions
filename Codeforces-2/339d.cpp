...
struct segment_tree{
    int n, size, log;
    vector<T> data;
    F TT1, TT2; // monoid operation (always adjacent)
    T T_id; // monoid identity
    // O(n)
    segment_tree(int m_n, F m_TT1, F m_TT2, T m_T_id): segment_tree(vector<T>(m_n, m_T_id), TT1(m_TT1), TT2(m_TT2), m_T_id){}
    // O(n)
    segment_tree(int m_n, T m_init, F m_TT1, F m_TT2, T m_T_id): segment_tree(vector<T>(m_n, m_init), TT1(m_TT1), TT2(m_TT2), m_T_id){}
    // O(n)
    segment_tree(const vector<T> &a, F m_TT1, F m_TT2, T m_T_id): n((int)a.size()), TT1(m_TT1), TT2(m_TT2), T_id(m_T_id){ // O(n)
    ...
    void refresh(int i){
        if ((log - __lg(i)) & 1) {
            data[i] = TT1(data[i << 1], data[i << 1 | 1]);
        } else {
            data[i] = TT2(data[i << 1], data[i << 1 | 1]);
        }
    }
    // O(log n)
    void set(int p, T x){
...
lld merge_or(lld a, lld b) {
    return a | b;
}


lld merge_xor(lld a, lld b) {
    return a ^ b;
}
...
void solveEachTest(int _TestCase) {
    ...
    segment_tree xorredSeg(arr, merge_or, merge_xor, 0ll);

    // NonIterable.print(xorredSeg.data);

    forn(qq, q) {
        int pos, val; cin >> pos >> val;
        xorredSeg.set(pos-1, val);
        // NonIterable.print(xorredSeg.data);
        cout << xorredSeg.data[1] << "\n";
    }
