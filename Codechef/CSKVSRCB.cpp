...
int arr[21][21], dp[21][21], n, m;

bool isValid(int x, int y) {
	return (x < n and y < m and x >=0  and y >= 0);
}

int solve(int currX, int currY, int endX, int endY) {
	if (currY >= m or currX >= n or currX < 0 or currY < 0) {
		return 0;
	}
	...
	dp[currX][currY] = 0;
	forn(i, 4) {
		if (isValid(currX+d4i[i], currY+d4j[i]) and (arr[currX+d4i[i]][currY+d4j[i]] == arr[currX][currY])) {
			dp[currX][currY] = max(dp[currX][currY], solve(currX+d4i[i], currY+d4j[i], endX, endY));
		}
	}
	return dp[currX][currY];
}
...
void solveEachTest(int _TestCase) {
    ...
    int stx, sty, endX, endY; read(stx, sty, endX, endY);
    ...
    int res = solve(stx, sty, endX, endY);

    println(!(res ^ arr[stx][sty]));
