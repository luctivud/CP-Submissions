...
void solveEachTest(int _TestCase) {
    ...
    // }
    if (n & 1) {
        // ans = (mod + power(2, n*k) - ans) % mod;
        forn(i, k) {
            ans += power(2, (k-i-1) * n) * ((power(2, n-1)) % mod);
            ans %= mod;
        }
        _____error_____(ans);
        ans = (mod + power(2, n*k) - ans) % mod;
        ...
        forn(i, k) {
            ans += mux * (power(2, k-i-1 * n));
            mux *= (mod + power(2, n-1) - 1) % mod;
            _____error_____(mux);
            ans %= mod;
        }
        ans += mux;
    ...
    //         break;
    //     }
    //     ans += (mux * ((n & 1) == 0) * (power(2, (k-i-1) * n)));
    //     _____error_____(mux, ans);
    //     mux = power(2, n-1) + (!(n & 1));
    // }
