...
int dpHorizontal[1001][1001], dpVertical[1001][1001];		
...
int checkScam(int i, int j) {
	bool ok = false;
	forn(xx, 4) {
		if (isvalid(i+d4i[xx]) and isvalid(j+d4j[xx])) {
			ok |= matrix[i][j] == matrix[i+d4i[xx]][j+d4j[xx]];
		}
	}
	return int(!ok);
}
...
void traverse(int i, int j, int &N, int &M) {
	visited[i][j] = true;
	N = max(N, dpHorizontal[i][j]); M = max(j, dpVertical[i][j]);
	forn(xx, 4) {
		if (isvalid(i+d4i[xx]) and isvalid(j+d4j[xx]) and (dpHorizontal[i+d4i[xx]][j+d4j[xx]] >= dpHorizontal[i][j]) and 
			dpVertical[i+d4i[xx]][j+d4j[xx]] >= dpVertical[i][j]) {
			traverse(i+d4i[xx], j+d4j[xx], N, M);
		}
	}
}
...
void solveEachTest(int _TestCase) {
    ...
    forn(i, n) forn(j, n) read(matrix[i][j]);

    int arr[n][n] = {0};
    forn(i, n) {
    	forn(j, n) {
    		arr[i][j] = checkScam(i, j);
    	}
    }
    ...
    forn(i, n) {
    	forn(j, n) {
    		if (i == 0) {
    			dpHorizontal[i][j] = arr[i][j];
    		} else {
    			dpHorizontal[i][j] = (arr[i][j] ? dpHorizontal[i-1][j] + arr[i][j] : 0ll);
    		}
    		if (j == 0) {
    			dpVertical[i][j] = arr[i][j];
    		} else {
    			dpVertical[i][j] = (arr[i][j] ? dpVertical[i][j-1] + arr[i][j] : 0ll);
    		}
    	}
    }
    ...
    forn(i, n) {
    	forn(j, n) {
    		if (!visited[i][j] and (dpHorizontal[i][j] == 1 or dpVertical[i][j] == 1)) {
    			int N = 1, M = 1;
    			traverse(i, j, N, M);
    			ans += countSquares(N, M);
    		} else if (!visited[i][j]) {
    			ans += 1;
    		}
    	}
    ...
    // forn(i, n) {
    // 	forn(j, n) {
    // 		print(dpHorizontal[i][j], "");
    // 	} println();
    // }
    // println();
    // forn(i, n) {
    // 	forn(j, n) {
    // 		print(dpVertical[i][j], "");
    // 	} println();
    // }
    // println(countSquares(3, 3));
