...
void solveEachTest(int _TestCase) {
    ...
    forn(i, n) {
        ...
        balls[i][2] = i + 1;
        if (balls[i][1]) {
            right.push_back(l - balls[i][0]);
        } else {
            left.push_back(balls[i][0]);
        }
    }

    sort(balls.begin(), balls.end(), [](array<lld, 3> &a, array<lld, 3> &b) {
        return a[0] < b[0];
    });

    _____error_____(balls);
    ...
    _____error_____(left);
    _____error_____(right);

    forn(i, (lld)left.size()) {
        ans.push_back({left[i], balls[i][2]});
    }

    forn(i, (lld)right.size()) {
        ans.push_back({right[i], balls[left.size() + i][2]});
    }

    _____error_____(ans);

    sort(ans.begin(), ans.end(), [](pair<lld, lld> &a, pair<lld, lld> &b) {
