...
void solveEachTest(lld _TestCase) {
	...
	vector<lld> kick(n, 0), start(n, 0);

	forn(i, n) {
		if (i + 3 < n and s[i] == 'K' and s[i + 1] == 'I' and s[i + 2] == 'C' and s[i + 3] == 'K') {
			kick[i] = 1;
		}
		if (i + 4 < n and s[i] == 'S' and s[i + 1] == 'T' and s[i + 2] == 'A' and s[i + 3] == 'R' and s[i + 4] == 'T') {
			start[i] = 1;
		}
	}
