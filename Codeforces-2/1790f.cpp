...
void dfs(lld node, lld prev, lld curSz, lld &minSz, vlld adj[], vlld &color) {
	
	// node, prev, currsz
	queue<array<lld, 3>>q;
	q.push({node, prev, 0});
	while (q.size()) {
		auto [nod, prv, cur] = q.front();
		q.pop();
		if (cur >= minSz) {
			return;
		}
...
void bfs(lld node, lld &res, vlld adj[], vlld &dist) {
	// node, prev, currsz
	queue<array<lld, 3>>q;
	q.push({node, 0, 0});
	while (q.size()) {
...
void solveEachTest(int _TestCase) {
	...
	vlld ans(n-1, 1ll);
	lld res = n;
	bfs(root, res, adj, dist);
	debugluc(dist);
	forn(i, n-1) {
		res = min(res, dist[ops[i]]);
		bfs(ops[i], res, adj, dist);
		ans[i] = (res);
		if (res == 1) break;
	}
	debugluc(dist);
