...
lld solve(lld node, vector<lld> adj[], vector<bool> &visited, map<pair<lld, lld>, lld> &count) {
	visited[node] = true;
	lld children = 1ll;
	EACH(nu, adj[node]) {
		if (!visited[nu]) {
			count[{min(node, nu), max(nu, node)}] = 1 + solve(nu, adj, visited, count);
			children += count[{min(node, nu), max(nu, node)}];
		}
	}
	return children;
}
...
void solveEachTest(lld _TestCase = 1) {
    ...
    solve(0, adj, visited, count);
    EACH(it, count) {
    	error(it.f1.f1, it.f1.s2, it.s2);
    }
