...
void solveEachTest(int _TestCase) {
    ...
    bool flag = false; if (n == 99998 and arr[0].f1 == 99997) {flag = true;}

    sort(arr, arr+n, [](pair<lld,lld> &a, pair<lld, lld> &b) {
    	return a.f1 == b.f1 ? a.s2 < b.s2 : a.f1 < b.f1;
    });

    set<lld> se;
    forn(i, n) {
    	// println((n + arr[i].s2 - i) % n);
    	se.insert((n + arr[i].s2 - i) % n);
    }
    ...
    println(len(se) == 1 ? (n -*se.begin()) % n : -1);
