...
int openTreasure(int input1, int input2, int **input3) {
    vector<int> locks(input1);
    for (int i = 0; i < input1; i++) {
        for (int j = 0; j < input2; j++) {
            locks[i] += input3[i][j] << j;
        }
    }
    int maxn = 1 << input2;
    vector<vector<int>> dp(input1+1, vector<int> (maxn+1, 0));
    dp[0][0] = 1;
    for (int i = 0; i < input1; i++) {
        for (int j = 0; j <= maxn; j++) {
            dp[i+1][j] += dp[i][j];
            dp[i+1][j|locks[i]] += dp[i][j];
        }
    }
    return dp[input1][maxn];
}
...
void solveEachTest(int _TestCase) {
    ...
    openTreasure(input1, input2, input3);
...
signed main() {
    ...
    for (int _TestCase = 1; _TestCase <= __NTestCases; _TestCase++) {
        ...
        // cerr << "Finished in : " << diff.count() << "\input1";
    #endif
