...
void solveEachTest(lld _TestCase) {
	// cout << "Case#" << _TestCase << ": ";
	lld n, m, g; read(n, m, g);
	vector<lld> time(n); read(time);
	vector<lld> clothes(m); read(clothes);

	lld maxTime = 0ll;
	forn1(i, n - 1) {
		maxTime = max(maxTime, time[i] - time[i - 1]);
	}

	// println(maxTime);
	sort(all(clothes));

	println((lld)(upper_bound(all(clothes), maxTime) - clothes.begin()));
