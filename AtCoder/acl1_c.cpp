...
lld mxTurn, nextI, nextJ;


void getNext(
    lld i, lld j, vector<vector<char>> &grid, lld turn, lld &newX, lld &newY) {
	if (i >= n or j >= m) {
		return;
	}
	// error(i, j, turn);
	...
	if (i + 1 < n and grid[i + 1][j] == '.') getNext(i + 1, j, grid, turn + 1, newX, newY);
	if (j + 1 < m and grid[i][j + 1] == '.') getNext(i, j + 1, grid, turn + 1, newX, newY);
...
void solveEachTest(lld _TestCase) {
	...
	vector<vector<char>> grid(n, vector<char>(m)); read(grid);
	...
	for4(i, n - 1, -1, -1) {
		for4(j, m - 1, -1, -1) {
			if (grid[i][j] == 'o') {
				...
				lld newX = i, newY = j;
				mxTurn = 0ll;
				getNext(i, j, grid, 0ll, newX, newY);
				grid[i][j] = '.';
				grid[newX][newY] = 'o';
				ANS += mxTurn;
			}
