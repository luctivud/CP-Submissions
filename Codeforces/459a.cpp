...
void solveEachTest(lld _TestCase) {
    // cout << "Case #" << _TestCase << ": ";
    lld x1, y1, x2, y2; read(x1, y1, x2, y2);

    lld diff1 = llabs(x1 - x2);
    lld diff2 = llabs(y1 - y2);
    if (x1 != x2 and y1!=y2 and llabs(diff1) != llabs(diff2)) {
    	println(-1);
    	return;
    }

    if (x1 == x2) {
    	println(x1+diff2, y1, x2+diff2, y2);
    } else if (y1 == y2) {
    	println(x1, y1+diff1, x2, y2+diff1);
    } else {
    	println(x1, y2, x2, y1);
    }
