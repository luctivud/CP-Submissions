...
void solveEachTest(int _TestCase) {
    // cout << "Case #" << _TestCase << ": ";
    PBSET <pair<lld, lld>> se;
    se.insert(make_pair(4, 5));
    se.insert(make_pair(5, 4));
    se.insert(make_pair(4, 4));

    EACH(it, se) {cout << it.f1 << " "<< it.s2 << "\n";}

    println(se.order_of_key(make_pair(4, 4)));
