...
void solveEachTest(lld _TestCase) {
	// cout << "Case #" << _TestCase << ": ";
	lld a, b, x1, y1, x2, y2; read(a, b, x1, y1, x2, y2);
	...
	lld firstX = (x1 + a - 1) / a;
	if (firstX * a == x1) firstX++;
	lld lastX = (x2) / a;
	lld num1 = (lastX - firstX + 1);
	// error(firstX, lastX);
	// for y
	...
	lld num2 = (lastY - firstY + 1);

	// error(x1, x2, y1, y2, num1, num2);

	println(max(num2, num1));
