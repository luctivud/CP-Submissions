...
void solveEachTest(int _TestCase) {
  ...
  forn(i, n) {
	...
	lld minAdd = INFll / 3, minRemove = INFll / 3;
  forn1(i, arr[n-1]) {
  	lld addBulb = 0ll, removeBulb = 0ll;
  	lld pos = lower_bound(all(arr), i) - arr.begin();
  	// error(i, pos);
  	addBulb += (pos * i) - pref[pos];
  	removeBulb += pref[n] - pref[pos] - ((n-pos) * i) ;
  	error(i, pos, addBulb, removeBulb);
  	if (addBulb + removeBulb < minAdd + minRemove) {
  		minAdd = addBulb;
  		minRemove = removeBulb;
  	} else if (addBulb + removeBulb == minAdd + minRemove) {
  		if (minAdd < addBulb) {
  			minAdd = addBulb;
  			minRemove = removeBulb;
  		}
