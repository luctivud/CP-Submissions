...
lld horz[501][501], vert[501][501];
lld dp[501][501][12];

// lld traverse(lld i, lld j, lld k) {
// 	return -1;
// }
    ...
    forn1(k, KK) {
	    forn(i, n) {
	    	forn(j, m) {
	    		// up
	    		if (isvalid(i-1, n))
	    		dp[i][j][k] = min(dp[i][j][k], dp[i-1][j][k-1] + vert[i-1][j]);
	    		// down
	    		if (isvalid(i+1, n))
	    		dp[i][j][k] = min(dp[i][j][k], dp[i+1][j][k-1] + vert[i][j]);
	    		// left
	    		if (isvalid(j-1, m))
	    		dp[i][j][k] = min(dp[i][j][k], dp[i][j-1][k-1] + horz[i][j-1]);
	    		// right
	    		if (isvalid(j+1, m))
	    		dp[i][j][k] = min(dp[i][j][k], dp[i][j+1][k-1] + horz[i][j]);
