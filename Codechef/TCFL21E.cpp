...
void solveEachTest(int _TestCase) {
    // cout << "Case #" << _TestCase << ": ";
    long double angle, limit; cin >> angle >> limit;
    long double ans = 0, factor = 180.00 / angle;

    lld g = __gcd(180ll, lld(angle));
    ...
    ans = lld(limit / factor2);

    lld yoyo = lld(round(ans));

    // println(yoyo);

    lld zz = 0ll;
	for4(i, 3, limit+1, 1) {
		long double Z = (180.00 / i);
		long double cut = (angle / Z);
		if (cut <= (i-2) and abs(lld(cut) - cut) <= 1e-6)
			zz += 1;
	}
	if (zz != yoyo)
	println(angle, limit, factor, zz, yoyo, zz == yoyo);

    // long double zz = lld((limit - 2) / 2);
    // bool ok = round(ans) == zz;
    // if (!ok)
    // println(ok);
