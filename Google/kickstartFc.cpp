...
void solveEachTest(lld _TestCase) {
	cout << "Case #" << _TestCase << ": ";
	string s = "021232";
	lld n, ra, pa, rb, pb, c; read(n, ra, pa, rb, pb, c);
	// error(n);
	vector<lld> adj[n * n];
	forn(i, 3) {
		lld a = s[2 * i] - '0';
	...
	vector<bool> obstacles(n * n);
	forn(i, c) {
		lld row, col; read(row, col);
		row--;
		lld temp = ((row * row) - 1) + col;
		// error(temp);
		obstacles[temp] = true;
	}
	...
	lld start = ((ra * ra) - 1) + pa;
	lld end = ((rb * rb) - 1) + pb;

	// error(start, end);

	if (start == 0) {
		if (end == 2 or obstacles[2]) {
			if (end == 2) {
				lld ans = 0ll;
				if ((!obstacles[1] and start != 1) or (!obstacles[3] and start != 3) or (!obstacles[0] and start != 0)) {
					ans--;
				}
				println(ans);
			} else {
			...
			lld ans = 1;
			if (end != 1 and !obstacles[1]) {
				ans++;
			} else if (end != 3 and !obstacles[3]) {
				ans++;
			}
			println(ans);
		}
		...
		lld ans = 0ll;
		if ((!obstacles[0] and end != 0) or (!obstacles[1] and end != 1) or (!obstacles[3] and end != 3)) {
			ans++;
		}
		println(ans);
	} else if (start == 3) {
