...
void solveEachTest(lld _TestCase) {
	// cout << "Case#" << _TestCase << ": ";
	lld n, k, p; read(n, k, p);

	vector<pair<lld, lld>> arr(n);
	forn1(i, n) {
		arr[i - 1] = {read(_TestCase), i};
	}
	...
	forn(i, n - 1) {
		if (arr[i + 1].f1 - arr[i].f1 <= k) {
			Union(arr[i].s2, arr[i + 1].s2, parent, rank);
		}
	}

	forn(qq, p) {
		lld a, b; read(a, b);
		println(find_repr(a, parent, rank) == find_repr(b, parent, rank) ? "Yes" : "No");
	}
