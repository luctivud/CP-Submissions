...
void solveEachTest(int _TestCase) {
    // cout << "Case #" << _TestCase << ": ";
    string s; cin >> s;
    vector<set<lld>> posOfChars(26);

    forn(i, len(s)) {
        posOfChars[s[i] - 'a'].insert(i);
    }
    ...
    forn(qq, q) {
        ...
        if (qType == 1) {
            ...
            cin >> pos >> c;
            --pos;
            posOfChars[s[pos] - 'a'].erase(pos);
            s[pos] = c;
            posOfChars[s[pos] - 'a'].insert(pos);
        } else {
            lld l, r; cin >> l >> r;
            --l, --r;
            lld ans = 0ll;
            forn(i, 26) {
                auto elPos = (posOfChars[i].lower_bound(l));
                if (elPos != posOfChars[i].end() and *elPos <= r) {
                    ans += 1;
                }
            }
            cout << ans << "\n";
