...
pair<lld, lld> merge_func(pair<lld, lld> a, pair<lld, lld> b) {
    return {a.f1 + b.f1, a.s2 + b.s2};
}
...
void solveEachTest(int _TestCase) {
    ...
    segment_tree segg(arr, merge_func, {0, 0});
    ...
    forn(qq, q) {
        ...
        if (QType == 0) {
            int pos, val;
            cin >> pos >> val;
            segg.set(pos-1, {val & 1, !(val & 1)});
        } else {
            int l, r; cin >> l >> r;
            auto [ax, bx] = (segg.query(l-1, r));
            if (QType == 1) {
                cout << bx << "\n";
            } else {
                cout << ax << "\n";
