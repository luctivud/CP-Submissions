...
bool isvalid(lld i, lld j) {
	return (j >= 0) and (i < n) and (ans[i][j] == -1);
}


void traverse(lld i, lld j, lld cnt, lld val) {
	if (cnt == 0) return;
	// println(i, j, cnt, val);
	ans[i][j] = val;
	cnt--;
	if (isvalid(i, j-1)) {
		traverse(i, j-1, cnt, val);
	} else if (isvalid(i+1, j)){
		traverse(i+1, j, cnt, val);
	}
}
...
// lld x = i, y = i, cnt = arr[i], val = arr[i];
    	// while ((cnt > 0) and (x >= 0) and (ans[x][y] == -1)) {
    	// 	ans[x--][y] = val;
    	// 	cnt--;
    	// }
    	// if (x == -1) {
    	...
    	// 	y++;
    	// }
    	// while ((cnt > 0) and (y <= n) and ans[x][y] == -1) {
    	// 	ans[x][y++] = val;
    	// 	cnt--;
    	// }
    	// if (y == n+1) {
