...
void dfs(lld node, lld parent, vector<lld> &par, vector<lld> &par2, vector<lld> adj[]) {
	for (auto i : adj[node]) {
		if (i != parent) {
			dfs(i, node, par, par2, adj);
		}
	}
	if (parent) {
		par[node] = parent;
...
void solveEachTest(int _TestCase) {
  ...
  vector<lld> par(n+1, 0), par2(n+1, 0);

  dfs(1, 0, par, par2, adj);

  set<lld> visited;
  forn1(i, n) {
  	if (len(adj[i]) == 1) {
  		// error(i, par[i], par2[i]);
  		if (par[i]) {
  			if (visited.find(par[i]) != visited.end()) {
  				ok = true;
  			} else {
  				visited.insert(par[i]);
  			}
  		} else if (par2[i]) {
  			if (visited.find(par2[i]) != visited.end()) {
  				ok = true;
  			} else {
  				visited.insert(par2[i]);
  			}
  		}
  	}
  }

  println(ok ? "SAFE" : "UNSAFE");
