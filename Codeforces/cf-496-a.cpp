...
void solveEachTest(lld _TestCase) {
	...
	for4(i, 1ll, n - 1ll, 1ll) {
		lld Ldiff, Rdiff, newDiff;
		Ldiff = arr[i] - arr[i - 1], Rdiff = arr[i + 1] - arr[i], newDiff = arr[i + 1] - arr[i - 1];
		diff.erase(diff.find(Ldiff)); diff.erase(diff.find(Rdiff));
		diff.insert(newDiff);
		ans = min(ans, (*diff.rbegin()));
		diff.erase(diff.find(newDiff));
		diff.insert(Ldiff); diff.insert(Rdiff);
	}
