...
lld solve(lld node, vector<lld> adj[], vector<lld> &sumAt, vector<lld> &maxAt, vector<bool> &visited) {
	// lld thisMaxAt = 0ll;
	visited[node] = true;
	sumAt[node] = 1ll;
	EACH(nu, adj[node]) {
		if (!visited[nu]) {
			lld thisNodeHas = 1 + solve(nu, adj, sumAt, maxAt, visited);
			maxAt[node] = max(maxAt[node], thisNodeHas);
			sumAt[node] += thisNodeHas;
		}
	}
	EACH(nu, adj[node]) {
		maxAt[nu] = max(maxAt[nu], maxAt[node]);
		// sumAt[nu] += sumAt[node];
	}
	return sumAt[node];
}
...
void solveEachTest(lld _TestCase = 1) {
    // cout << "Case#" << _TestCase << ": ";
    lld n; read(n);
    vector<lld> sumAt(n, 0);
    vector<lld> maxAt(n, 0);
    vector<lld> adj[n];
    ...
    solve(0, adj, sumAt, maxAt, visited);
    debspace(maxAt);
    debspace(sumAt);
