...
void solveEachTest(lld _TestCase) {
    ...
    forn(i, m) {
    	lld temp_a, temp_b; read(temp_a, temp_b);
    	Query qq; 
    	qq.number = i, qq.l = --temp_a, qq.r = --temp_b;
    	queries[i] = qq;
    }

    lld block = (lld)sqrtl(n);
    ...
    lld lc = 0ll, rc = 0ll;
    forn(i, m) {
    	Query this_query = queries[i];
    	lld ind = this_query.number;
    	lld l = this_query.l;
    	lld r = this_query.r;
    	while (lc < l) {
    		count[arr[lc]]--;
    		if (count[arr[lc]] == arr[lc]) {
    			cnt++;
    		} else if (count[arr[lc]] == arr[lc]-1) {
    			cnt--;
    		}
    		lc++;
    	}
    	...
    	while (rc-1 > r) {
    		rc--;
    		count[arr[rc]]--;
    		if (count[arr[rc]] == arr[rc]) {
    			cnt++;
    		} else if (count[arr[rc]] == arr[rc] -1) {
    			cnt--;
    		}
    	}
