...
void solveEachTest(int _TestCase) {
    ...
    lld n, a, b; cin >> n >> a >> b;
    n /= 2;
    vector<lld> arr(2*n); forn(i, 2*n) cin >> arr[i]; 
    sort(all(arr));
    vector<lld> pref(n+1, 0ll), suff(n+1, 0ll);
    forn(i, n) {
        pref[i+1] += pref[i] + arr[2*i];
        suff[n-i-1] += suff[n-i] + arr[2*(n-i)-1];
    }

    bool ok = false;
    _____error_____(pref);
    _____error_____(suff);

    forn(i, n+1) {
        lld sumz = (pref[i] + suff[i]);
        ok |= (sumz >= a and sumz <= b);
    }
    cout << (ok ? "Alice" : "Bob");
    return;
}
