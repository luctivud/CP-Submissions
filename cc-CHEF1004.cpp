...
bool isvalidInd(lld x, lld n) {
	return (x >= 0 and x < n);
}


lld solve(lld n, lld m, vector<lld>  arr, vector<vector<lld>> &dp) {
	if (m == 0) {
		return 1;
	}
	...
	dp[n][m] = solve(n-1, m, arr, dp) + solve(n-1, m-arr[n], arr, dp);
...
void solveEachTest(lld _TestCase = 1) {
	...
	vector<vector<lld> > dp(sz, vector<lld> (x + 1, -1));
	println(solve(sz-1, x, arr, dp));
